...
async function bootstrap() {
  try {
    ...
    console.log('Service findCredentialsByUserId result:', creds1 ? 'Found' : 'Not Found');

    // Try direct model access
    const userCredentialModel = (usersService as any).userCredentialModel;
    
    // Query with 'userId' (property name)
    const creds2 = await userCredentialModel.findOne({ userId: user._id }).exec();
    console.log('Query with { userId: ... } result:', creds2 ? 'Found' : 'Not Found');
    
    // Query with 'user_id' (DB field name)
    const creds3 = await userCredentialModel.findOne({ user_id: user._id }).exec();
    console.log('Query with { user_id: ... } result:', creds3 ? 'Found' : 'Not Found');
